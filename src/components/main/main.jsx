...
import {getUnseenMessagesNumSelector} from "../../store/selectors";
import Controls from "../controls/controls";
import MessagesList from "../messages-list/messages-list";

const Main = () => {
  const unseenMessagesNum = useSelector((state) =>
    getUnseenMessagesNumSelector(state)
  );
  ...
  const handleSubmit = useCallback((newMessageText) => {
    const newMessage = {
      id: nanoid(ID_LENGTH),
      text: newMessageText,
      seen: isListVisible,
  ...
  const debouncedHandleDeleteAll = useCallback(
      debounce(() => handleDeleteAll()),
      []
  );
    ...
    <div className="main-container">
      <div className="board">
        <div className="board__header header">
          ...
          >
            {unseenMessagesNum > 0 && (
              <div className="header__icon-indicator">{unseenMessagesNum}</div>
            )}
          </div>
        </div>
